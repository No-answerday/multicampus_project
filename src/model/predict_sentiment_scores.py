...
def predict_sentiment_scores(model, df):
    """
    ...
    Returns:
        int: 업데이트된 리뷰 개수
    """
    print("\nsentiment_score 예측 중...")

    # word2vec이 None이 아닌 행만 필터
    valid_mask = df["word2vec"].notna()
    ...
    X = np.array(df.loc[valid_indices, "word2vec"].tolist())
...
def process_category_files(model, partitioned_reviews_dir):
    """
    ...
    for file_path in parquet_files:
        ...
        # sentiment_score 예측
        updated = predict_sentiment_scores(model, df)
...
def main():
    ...
    """
    print("=" * 70)
    print("Sentiment Score 예측 및 업데이트")
    print("=" * 70)

    # 경로 설정
    MODEL_PATH = "./models/logistic_regression_sentiment.joblib"
    ...
    # 2. 카테고리별 파일 처리
    total_updated = process_category_files(model, PARTITIONED_REVIEWS_DIR)
